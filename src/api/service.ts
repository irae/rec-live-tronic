...
type RecordingPatch = { title?: unknown; start_at?: unknown; stop_at?: unknown };
...
export class RecorderService {
  ...
  async patchRecording(id: string, input: RecordingPatch): Promise<{ recording: ReturnType<typeof mapRecording>; runtime_updated?: boolean; relaunched?: boolean; stop?: { attempted: true; confirmed: boolean } }> {
    ...
    const patch: { title?: string; startAt?: string; stopAt?: string } = {};
    ...
    if (existing.status === "scheduled") {
      ...
      if (patch.title !== undefined || patch.startAt !== undefined || patch.stopAt === undefined) throw new AppError("STATUS_CONFLICT", 409, "Running recordings may only change stop_at");
